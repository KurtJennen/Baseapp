...
public class BatchJobExecutionJmesaTemplate extends CustomTableFacadeTemplate {
	...
	public BatchJobExecutionJmesaTemplate(final TableFacade tableFacade, final ArrayList<?> items, final HttpServletRequest request) {
	...
	protected void addFilterMatchers(final Map<MatcherKey, FilterMatcher> filterMatchers) {
		...
		filterMatchers.put(new MatcherKey(Date.class, getColumnProperties()[BaseConstants.VIER]), new DateFilterMatcher(DateTool.UTIL_DATETIME_PATTERN));
		filterMatchers.put(new MatcherKey(Date.class, getColumnProperties()[BaseConstants.ACHT]), new DateFilterMatcher(DateTool.UTIL_DATETIME_PATTERN));
	}
